...
def translate_points(pointarray,matrix):
    ...
    return numpy.asarray(old_div(tpt[:,:3],tpt[:,3:4]),numpy.float32)
...
def calculate_normals_by_cross_product(pointarray,facearray,smarray):
    ...
    lq = old_div(1.0, numpy.sqrt(q[qnz]))
...
def calculate_normals_by_angle_subtended(pointarray,facearray,smarray):
    ...
    rnorms[0::3][ndg] = p*(old_div(angA,lp))[:,numpy.newaxis]
    rnorms[1::3][ndg] = p*(old_div(angB,lp))[:,numpy.newaxis]
    rnorms[2::3][ndg] = p*(old_div(angC,lp))[:,numpy.newaxis]
