...
class FileLikeBuffer(object):
    ...
    def read_to_nul(self):
        try:
            i = self.buf.index(b'\0',self.index,self.end)
        except IndexError:
...
class ChunkBase(with_metaclass(ChunkMetaclass, object)):
    """Base class of chunks.
    ...
    def document_html(cls,flo,done):
        ...
        d = {}
        for typ,attr in list(cls.single_types.items()):
            if attr not in d:
                d[attr] = []
            d[attr].append(typ)
        ...
        d = {}
        for typ,attr in list(cls.multiple_types.items()):
            if attr not in d:
                d[attr] = []
            d[attr].append(typ)
...
class UndefinedChunk(with_metaclass(UndefinedChunkMetaclass, ChunkBase)):
    def read(self,fbuf,flags):
